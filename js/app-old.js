...
const util = (() => {
    ...
    const timer = () => {
        let countDownDate = (new Date(document.getElementById('time-display').getAttribute('time-data').replace(' ', 'T'))).getTime();
        ...
        setInterval(() => {
            ...
            document.getElementById('oclock').innerText = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    ...
    const guest = () => {
        ...
        if (!name) {
            document.getElementById('guest-name').remove();
            return;
        }
        ...
        document.getElementById('guest-name').appendChild(div);
    };
...
const pagination = (() => {
    ...
    const buttonAction = async (button) => {
        ...
        document.getElementById('speech-list').scrollIntoView({ behavior: 'smooth' });
...
const like = (() => {
    ...
    const like = async (button) => {
        ...
        if (!token) {
            alert('There is an error, token is empty!');
            window.location.reload();
            return;
        }
            ...
            await request('PATCH', '/api/comment/' + likes.get(id))
                ...
                .then((res) => {
                    if (res.data.status) {
                        ...
                        info.setAttribute('like-data', (parseInt(info.getAttribute('like-data')) - 1).toString());
                    }
                })
                .catch((err) => {
                    alert(`There is a mistake: ${err}`);
        ...
        info.innerText = info.getAttribute('like-data') + ' like';
        button.disabled = false;
    };
...
const comment = (() => {
    ...
    const resetForm = () => {
        ...
        document.getElementById('label-presence').style.display = 'block';
        document.getElementById('reply').innerHTML = null;
    ...
    const balasan = async (button) => {
        ...
        document.getElementById('reply').innerHTML = renderLoading(1);
        formKehadiran.style.display = 'none';
        document.getElementById('label-presence').style.display = 'none';
